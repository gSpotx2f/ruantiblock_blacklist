...
ClearDataFiles() {
    if [ -d "$DATA_DIR" ]; then
        ...
        printf "" > "$IP_DATA_FILE"
        printf "0 0" > "$UPDATE_STATUS_FILE"
        printf "" > "$USER_ENTRIES_STATUS_FILE"
    fi
}

AddUserEntries() {
    local _return_code=0 _ip_data_file _dnsmasq_data_file _user_entries_status_file _str _update_string
    ...
    if [ "$ADD_USER_ENTRIES" = "1" ]; then
        ...
        export I_INSTANCE_ENTRIES_FILE="$USER_ENTRIES_FILE"
        ...
        do
            _update_string=$(printf "$_str" | $AWK_CMD '{
                if(NF == 3) {
                    printf "User entries (%s): IP: %s, FQDN: %s", $3, $1, $2;
                };
            }')
